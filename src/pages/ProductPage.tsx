  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <Link to="/catalog">
          <Button className="bg-blue-4 hover:bg-teal-600 text-white">Вернуться в каталог</Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="text-sm text-gray-500 mb-6">
        ...
        <Link to="/catalog" className="hover:text-blue-4">
          Каталог
        </Link>
        <span className="mx-2">›</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div>
          <div className="bg-gray-50 rounded-lg p-4 mb-4 flex justify-center">
            <img src={productImages[selectedImage]} alt={product.name} className="max-h-[400px] object-contain" />
          </div>
          <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
            {productImages.map((image, index) => (
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
              >
                <img src={image} alt={`Миниатюра ${index + 1}`} className="w-full h-full object-contain" />
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="flex space-x-4 mb-8">
            ...
            <motion.button
              className={`w-12 h-12 rounded-full border flex items-center justify-center ${
                isFavorite(product.id) ? 'bg-red-50 border-red-300 text-red-500' : 'border-gray-300 text-gray-400'
              }`}
              onClick={toggleFavorite}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
            >
              <HeartIcon className={`h-6 w-6 ${isFavorite(product.id) ? 'fill-red-500 text-red-500' : ''}`} />
            </motion.button>
        ...
        <div className="flex overflow-x-auto border-b scrollbar-hide">
          <button
            className={`px-6 py-3 text-sm font-medium whitespace-nowrap ${
              activeTab === 'parameters' ? 'text-blue-4 border-b-2 border-blue-4' : 'text-gray-600 hover:text-blue-4'
            }`}
          ...
          <button
            className={`px-6 py-3 text-sm font-medium whitespace-nowrap ${
              activeTab === 'description' ? 'text-blue-4 border-b-2 border-blue-4' : 'text-gray-600 hover:text-blue-4'
            }`}
        ...
        <div className="pt-6">
          ...
          {activeTab === 'delivery' && (
            <div className="prose max-w-none">
              <h3>Доставка</h3>
              <p>
                Мы осуществляем доставку по всей России. Доставка по Москве и Санкт-Петербургу - бесплатно. Доставка в
                другие регионы осуществляется транспортными компаниями СДЭК, DPD и Почтой России.
              </p>
